...
sealed trait LinearTreePart {
  def lowerBound: Int = {
    this match {
      ...
      case LinearTreePartPred(wordPred) => 1
    }
  }
}
...
final case class LinearTreePartPred(wordPred: WordPred) extends LinearTreePart
...
object LinearTreePart {
  def fromRulePart(rulePart: RulePart): LinearTreePart = {
    rulePart match {
      ...
      case RulePartPred(wordPred) => LinearTreePartPred(wordPred)
    }
  }
}
